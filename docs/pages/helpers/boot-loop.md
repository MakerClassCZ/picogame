...
- `setup(display=None, strip_h=None, background=0, fast=True, top=0, bottom=0, left=0, right=0, rgb444=None)` - returns `(scene, buffer_a, buffer_b)`. On an SPI display it disables automatic refresh, clears `root_group`, and allocates two full-width render strips. On a framebuffer target such as Fruit Jam DVI or the browser playground, the scene composites into the framebuffer and both returned buffers are `None`. The `Scene` keeps any allocated strip buffers alive. A game that only calls `scene.refresh()` can ignore the returned buffers: `scene, _, _ = picogame_game.setup(...)`.
  ...
  - `rgb444` - `True` requests 12-bit colour on a compatible fast SPI backend. Use `"auto"` to enable it only when the board reports support. The default `None` reads `PICOGAME_RGB444` from `settings.toml`: `1` acts as `"auto"`, unset or `0` keeps RGB565 - the per-board switch for a transfer-bound panel such as the PyBadge. An explicit argument wins over the setting. Framebuffer targets ignore this setting. See [/hardware/](/hardware/).
