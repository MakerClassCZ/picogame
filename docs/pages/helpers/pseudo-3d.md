...
- `.draw(view, vx, vy, vw, vh)` - the `StripDraw` callback: paints the sky/floor bands, then the pre-merged wall runs with one `Canvas.vspans` batch call. **Row 0 is the top of the raycast VIEW, not of the screen** (the horizon sits at half the `sh` you gave `cast()`). If the layer does not start at screen `y=0` - the reserved-HUD-band pattern, `setup(top=BAND)` + `StripDraw(cb, 0, BAND, W, H - BAND)` - pass `vy - BAND`, or the whole picture renders BAND pixels too high: its top hides under the HUD and a strip of bare floor colour is left at the bottom. The alternative is a full-screen view (`cast(..., H)`, layer at `y=0`) with the HUD floating over it as fixed `SceneLabel`s - it costs no view height, which suits a first-person game.
