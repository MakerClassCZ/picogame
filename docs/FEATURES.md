...
| author many levels | the declarative scene format + editor; hand-code small ones | [Scene format](SCENE_FORMAT.md) · [Building scenes](/helpers/building-scenes/) |
...
| shrink a tileset | `png2picogame.py --dedup` (merges rotated/mirrored tiles) | [Engine guide](PICOGAME.md) |
| understand fast DMA vs portable rendering | `pg.Display` backend vs plain busdisplay | [Engine guide](PICOGAME.md) |
...
| run on a board I wired myself | prebuilt generic firmware + `settings.toml` (`PICOGAME_BUTTONS`, display, matrix, USB keys) | [Custom board](CUSTOM_BOARD.md) |
