...
- `Raycaster(world, wall_colors, sky, floor, fov=0.66, stride=2)` · `.cast(px, py, ang, sw, sh)` (once/frame) · `.draw(view, vx, vy, vw, vh)` (StripDraw callback) · `.solid(x, y)` (wall test) · `.attach(sd)` (temporal repaint) · `.project_sprite(sx, sy)` (billboard) — fully native render: the `pg.raycast` caster (integer 16.16 C on device, Python in the sim) also emits the RLE-merged wall runs, painted with one `Canvas.vspans` batch per strip into a 0-RAM `StripDraw` view (~36 fps uncapped at stride 1 full-screen on RP2040, flat across view angles). `stride` = perf/quality knob; `attach(sd)` + `always_dirty=False` repaints only the changed column band (still/slow ~30 fps). See [/helpers/pseudo-3d/](/helpers/pseudo-3d/).
