...
- `SceneBox(scene, pg, font, x, y, w, h, fg, bg, nlines=3, key=None, border=None)` · `.show(lines)` · `.hide()` · `.set_line(i, text)` · `.destroy()` — a multi-line in-scene panel (dialog/log); a row holds `(w - 16) // 6` chars of the 6 px font (8 px padding each side), rows 12 px apart (`h >= 14 + 12 * nlines`); longer lines are not clipped (the sim warns). destroy() = one-shot teardown (needs firmware with `Scene.remove`).
