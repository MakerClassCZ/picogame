...
- `remove(item)` — unlink a previously added item; the removal forces a FULL repaint next refresh (no ghost). `Scene.invalidate()` takes NO arguments — the sub-rect form belongs to `StripDraw.invalidate(x, y, w, h)` (all four positionally, or none).
...
`gravity` is px/tick² added to vy each tick, clamped to **-128..127.99**; NEGATIVE gravity is legal and makes particles RISE (embers, bubbles). Particle positions are **world** coordinates - `scene.set_view` moves them like any other layer.
...
- `vblank(framebuffer)` — (DVI boards, RP2350; **fork-only, see the note under `core1`**) pass the `picodvi.Framebuffer` the display scans out of; blocks until its next vertical blanking (≤ ~16.7 ms). Starting a full-frame compose right after vblank keeps the publish front consistently behind the beam, so each sweep shows one **whole** frame — removes single-buffer tearing while the compose fits within two sweeps. Costs the wait: budget it against your FPS cap.
...
- **`core1` and `vblank` are NOT in a CircuitPython release.** Both live on the fork's `picogame-core1` branch and have not gone upstream, so `pg.core1` and `pg.vblank` raise `AttributeError` on any firmware you download from circuitpython.org. Guard them with `hasattr(pg, "core1")` / `hasattr(pg, "vblank")` and treat the dual-core and tear-free paths as optimisations you may not have.
...
- `setup(display=None, strip_h=None, background=0, fast=True, top=0, bottom=0, left=0, right=0, rgb444=None) -> (scene, buffer_a, buffer_b)` — take over the display, build a Scene + two strip buffers. `top/bottom/left/right` reserve fixed HUD margins; `rgb444=True` opts into 12-bit colour on a supporting SPI panel, and `rgb444="auto"` enables it only where the board reports `picogame.RGB444_SUPPORTED`. The default `None` follows `PICOGAME_RGB444` in `settings.toml` (`1` = `"auto"`, unset = RGB565).
...
- `Timer(frames)` — input-leniency window (coyote time / jump buffering): `.feed(condition)` (recharge while true, else decay) · `.charge()` (fill) · `.is_active` · `.consume()` (true once, then clears) · `.t` (frames left; there is no `reset()` — `consume()` or `.t = 0` clears it, e.g. the coyote timer right after a jump so it can't fire twice).
...
### `picogame_i2cpad` — I2C gamepad source (any board with I2C, incl. the PicoPad)
- `I2CPad(recipe, i2c=None, address=None)` — a button **source** for `Buttons(sources=[…])` reading a "dumb" I2C button device: GPIO expanders (TCA9555, PCF8574, MCP23017) and vendor pads built on them. Needs no USB host, so it is the way to put an external controller on a PicoPad or a bare Pico. `.read()` → logical mask, `.mapped` → the buttons it can report, `.led(n, on)` → player LED (presets that have one).
- **Opt-in** via `settings.toml` — an expander has no identity register, so auto-probing could bind an unrelated device: `PICOGAME_I2CPAD = "qwstpad"` (preset — `"gamepadqt"` is the Adafruit Mini I2C Gamepad, whose thumbstick is read as the directions), `"qwstpad@0x23"` (address), `"qwstpad;qwstpad@0x23"` (several pads), or a full recipe `"addr=0x20 read=:1 inv=1 UP=0 A=4 …"`. `PICOGAME_I2C = "SDA,SCL"` names the bus pins on a bare board (or a single token names a board bus, e.g. `"I2C0"`). `Buttons()` attaches whatever is listed — see [Input](/helpers/input/).
...
- `TiltPad(recipe, i2c, addr=None)` — a button **source** whose directions are OR'ed with the real D-pad, so a handheld with a motion sensor steers by tilting and games need no changes. `.read()` → logical mask. It reads the sensor's registers directly: there is no driver to install.
...
- `attach(spec, i2c=None)` — the pad for a settings value; this is the one that defaults the bus (`TiltPad` itself wants an explicit `i2c`). `parse_recipe(text)` / `PRESETS`.
...
- `OptionsMenu(scene, pg, font, x, y, w, rows, fg, bg, title=None, border=None, visible=None)` · `.value(key)` · `.show(sel=0)` · `.hide()` · `.set_rows(rows, sel=0)` · `.tick(btn)` — an in-scene options screen of toggles/choices. **Its `tick()` returns the row's KEY where `SceneMenu`/`Menu` return an INDEX** - the one difference that bites when you move a menu from one to the other. Use `OptionsMenu` for named settings with values, `SceneMenu` for a plain list of actions. `visible` is a **row count**, not a flag: it fixes the window height up front (a list that grows and shrinks keeps one panel size); omitted, the panel fits the rows it was given.

### `picogame_shapes` — single-colour bitmap generators
- `rect(w, h, color)` · `circle(d, color)` · `ring(d, color, thickness=2)`
- `from_mask(mask, color)` — Bitmap from a **list of strings**, one per row (`'#'`, `'X'` or `'1'` = set); sized to the mask. `color` may also be a **{character: colour} dict** - then every character is its own palette entry (unlisted characters, e.g. `.` or space, stay transparent), so ONE mask carries body + outline + highlight: `from_mask(["  ##  ", " #oo# ", "#o..o#"], {"#": OUTLINE, "o": BODY, ".": GLINT})`. That is how you get the shape AND colour identity the design bar asks for without hand-building a PAL8 atlas. Passing a single string is not an error — each CHARACTER becomes a row, so you get a 1-pixel-wide sprite and no exception.
...
- Waiting primitives (use with `yield from` inside a script): `.text(lines)` (dialog page, A advances) · `.ask(lines) -> sets .answer` (A/B choice) · `.wait(frames)` · `.fade_out(speed)` / `.fade_in(speed)` · `.goto(level, at=None)` (fade out, set `.pending`, wait for the game loop to swap the level, fade in).
- Story flags: `.ev(name)` / `.set(name)` (sets AND replays the level's effects through `.on_flag`) / `.ev_set(name)` (plain) / `.ev_clear(name)`, kept in `.events` (a set — persist it via your save schema). `.view` = the current View (kept fresh by `picogame_story`). `.retarget(scene)` re-points the Director after a map change; `.pending` = `(level, at)` a script asked for.
...
- `Shake(scene, max_offset=6, decay=0.03, seed=0x9E37)` · `.add(amount)` (0.6 small kick, 0.8 hit, 1.0 big; under ~0.5 is sub-pixel at `max_offset=6`, i.e. invisible) · `.tick(cam_x=0, cam_y=0)` — trauma screen shake composed on top of the camera. `scene=None` = offset-only mode for strip-rendered games (road/raycaster/mode-7, which `set_view` never moves): tick() just updates `.ox`/`.oy` and you spend them in your renderer's params — `road.tick(dist, lateral + sh.ox)`, a jittered horizon.
...
- `make_output(sample_rate=22050, pin=None)` — returns this board's audio output, chosen automatically: an I2S DAC (Fruit Jam TLV320) when the board has `I2S_BCLK` and no `pin` was passed, else the analogue path on `pin` (or the board default) — PWM where the firmware has `audiopwmio`, the chip's true DAC where it does not (SAMD51: PyBadge, PyGamer). Used by both `picogame_audio` and `picogame_synth`, so a game needs no board-specific audio code. Raises `RuntimeError` if no output exists.
...
- `Synth(pin=None, sample_rate=22050, buffer_size=2048, music_level=0.4, sfx_level=0.7)` · `.sfx(n)` · `.sfx_seq(events, priority=0, window=0)` (timed chain — `events` = `[(delay_frames, voice), …]`, the trigger goes through `sfx()` arbitration and the tail is read from your list, no per-fire allocation) · `.press(n)` · `.release(n)` · `.music(midi_track)` · `.stop_music()` · `.set_levels(music=None, sfx=None)` · `.mute(on)` · `.deinit()` — release the output (PWM pin / I2S bus), mixer and synthesizer so a later `Synth()` / `Audio()` in the same program can claim them; idempotent, the instance becomes the silent no-op · `.available` — self-guarding init: on audio-less firmware **or** a failed init (tight heap, claimed pin) the instance runs as silent no-ops instead of raising; no try/except needed in games.
...
- `View`: `.tile_size` (the primary tilemap's `(tile_w, tile_h)` — what probes and offsets need) · `.tile_xy(px, py)` · `.group(tag)` (tagged single sprites included) · `.point(name)` · `.in_zone(x, y, tag=None)` · `.is_solid(tx, ty)` · `.tile_has(tx, ty, prop)` · `.play(sound_id)` · `.tick(dt)` · `.set_tile_prop(tile, prop, on=True)` · `.swap_tiles(a, b)` · `.effects` (story rules) · `.world` (authored world size) · `.name`.
...
- `bake_sounds(sounds, base=None) -> {id: path}` (wavs stay wav, loaded at runtime) · `bake_bank(assets, sounds=None, base=None) -> BANK` · `bake_level(level, size, assets) -> LEVEL` · `bake(scene) -> SCENE` (a standalone v1 scene). Colour assets are generated; a PNG asset becomes a `pal8f` reference to its `<name>.pal8` sidecar (no PIL on the device); `pal8_inline` (playground handoff) passes through.
...
- `encode_pal8(data, fw, fh, frames, palette, transparent) -> bytes` — a `.pal8` file (16-byte header, palette, indices; the constants `PAL8_MAGIC` / `PAL8_HDR` / `PAL8_HDR_LEN` are that header, spelled out in [Scene format](scene-format.md)). Costs ~3 kB while imported; `Game` drops it after boot.
...
- `Road(pg, width, height, horizon, colors, *, half_width=0.47, hw_min=6.0, depth=600.0, curves=((16384, 90.0), (4096, 30.0)), world_step=6, curve_step=2, hill_amp=0, edge_frac=0.12, dash_frac=0.07, dash_min_hw=7.0, band=20.0, dash_band=14.0)` · `.tick(dist, lateral_px=0)` (once/frame; positive lateral = the car moved right, the road shifts left) · `.draw(view, vy)` (StripDraw callback body) · `.set_grade(g)` (hills: −1..+1 moves the horizon; needs `hill_amp`) · `.horizon_now` (this frame's effective horizon = horizon + hill pitch — the y overlays and roadside sprites anchor to) · `.curve_at(dist)` (signed curvature −1..+1 for centrifugal pull / AI — the same two-sine model the C runs, zero-alloc) · `.row_of(z)` / `.half_of(row)` / `.edges_of(row)` (place and scale sprites ON the road — the rows are linear, so scale by `half_of`, not `F/(F+z)`) — drives the native `pg.road_edges` + `Canvas.road` pair from human units: curve `periods` in world units (rounded to the nearest power of two so the int32 phase wrap stays continuous — see the [calling contract](/helpers/pseudo-3d/)), `swing` in px of lateral bend, colors as a dict of six `pg.rgb565` values. Builds every fixed-point table once; `tick()` allocates nothing. See [/helpers/pseudo-3d/](/helpers/pseudo-3d/).

### `picogame_iso` — isometric projection
- `IsoView(ox, oy, tw, th)` (`tw`/`th` = tile half-width/half-height; 2:1 diamond → `th = tw//2`) · `.to_screen(gx, gy, h=0)` · `.depth(gx, gy, h=0)` (back-to-front painter's key) · `.screen_to_grid(sx, sy)` · `.cube_faces(gx, gy, height_px)` (top/right/left faces of a raised block) · `.emit_blocks(cells, tv, tc)` (alloc-free batch: writes flat-shaded cube triangles for many blocks straight into int16/uint16 buffers for ONE `Canvas.fill_triangles` call; returns the triangle count) — the **cheapest pseudo-3D there is**: integer add/shift only, no divide, no C dependency, which is why it runs well on the RP2040. Unlocks iso RPG / strategy / tactics / builder. **Draw the blocks as Sprites** (one baked Bitmap per block type, added in `depth()` order) unless the geometry is procedural: measured on a PicoPad with 150 blocks all moving, Sprites in a Scene cost 40 ms/frame against 89 ms for an `emit_blocks` + `Triangles` rebuild. Static boards are cheaper still: render once + dirty-rect the movers (30 fps). `emit_blocks` is the batch builder for scenes whose geometry really is computed per frame (~2× faster than a Python `cube_faces` loop). See [/helpers/pseudo-3d/](/helpers/pseudo-3d/).
