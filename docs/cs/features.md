...
| izometrická deska (RPG / taktiky / builder) | `picogame_iso.IsoView` na projekci a painter's klíč; bloky kresli jako **Sprites ve scéně** (zapečený art na typ bloku), nebo přes `emit_blocks` a trojúhelníky, když je geometrie procedurální | [Pseudo-3D](/cs/helpers/pseudo-3d/) |
...
| hrát s I2C gamepadem (QwSTPad a spol. — jakákoli deska s I2C, včetně PicoPadu) | `picogame_i2cpad` (opt-in přes `settings.toml`) | [Vstup a ovládání](/cs/helpers/input/) |
