...
- `Shake(scene, max_offset=6, decay=0.03, seed=0x9E37)` · `.add(amount)` (0.6 zásah, 0.15 náraz) · `.tick(cam_x=0, cam_y=0)` — trauma screen shake složený nad kamerou. `scene=None` = režim jen-offset pro strip renderery (road/raycaster/mode-7, kterými `set_view` nehne): tick() jen aktualizuje `.ox`/`.oy` a ty je utratíš v parametrech svého rendereru — `road.tick(dist, lateral + sh.ox)`, roztřesený horizont.
