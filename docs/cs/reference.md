...
- **`core1` ani `vblank` nejsou v žádném releasu CircuitPythonu.** Oba žijí na větvi `picogame-core1` v našem forku a do upstreamu zatím nešly, takže `pg.core1` i `pg.vblank` na firmwaru staženém z circuitpython.org vyhodí `AttributeError`. Ošetři je přes `hasattr(pg, "core1")` / `hasattr(pg, "vblank")` a ber dvoujádrovou i bez trhání cestu jako optimalizaci, kterou možná nemáš.
...
- `setup(display=None, strip_h=None, background=0, fast=True, top=0, bottom=0, left=0, right=0, rgb444=None) -> (scene, buffer_a, buffer_b)` — vybere backend a vytvoří scénu. Na SPI backendu vrátí dva strip buffery, na framebufferu dvakrát `None`. `top/bottom/left/right` rezervují okraje HUD; `rgb444=True` zapne 12bitové barvy na podporovaném SPI panelu a `rgb444="auto"` se řídí `RGB444_SUPPORTED`. Výchozí `None` se řídí klíčem `PICOGAME_RGB444` v `settings.toml` (`1` = `"auto"`, nenastaveno = RGB565).
...
- **Opt-in** přes `settings.toml` — expandér nemá identifikační registr, takže by oťukávání adres mohlo sáhnout na cizí zařízení: `PICOGAME_I2CPAD = "qwstpad"` (preset — `"gamepadqt"` je Adafruit Mini I2C Gamepad, jehož páčka se čte jako směry), `"qwstpad@0x23"` (adresa), `"qwstpad;qwstpad@0x23"` (více padů), nebo celý recept `"addr=0x20 read=:1 inv=1 UP=0 A=4 …"`. `PICOGAME_I2C = "SDA,SCL"` pojmenuje piny sběrnice na holé desce (jediný token místo toho pojmenuje sběrnici desky, např. `"I2C0"`). `Buttons()` připojí, co je vyjmenované — viz [Vstup](/cs/helpers/input/).
...
- `TiltPad(recipe, i2c, addr=None)` — zdroj tlačítek, jehož směry se sčítají se skutečným D-padem, takže handheld s pohybovým čidlem řídíš nakláněním a hry se nemusí měnit. `.read()` → logická maska. Čte registry čidla přímo, žádný ovladač se neinstaluje.
...
- `attach(spec, i2c=None)` — pad pro hodnotu z nastavení; sběrnici doplňuje právě tahle funkce (`TiltPad` sám chce `i2c` předat). `parse_recipe(text)` / `PRESETS`.
...
- `OptionsMenu(scene, pg, font, x, y, w, rows, fg, bg, title=None, border=None, visible=None)` · `.value(key)` · `.show(sel=0)` · `.hide()` · `.set_rows(rows, sel=0)` · `.tick(btn)` — **jeho `tick()` vrací KLÍČ řádku, zatímco `SceneMenu`/`Menu` vrací INDEX** (jediný rozdíl, který kousne při přechodu z jednoho na druhý) — obrazovka nastavení (přepínače/volby) ve scéně. `visible` je **počet řádků**, ne přepínač: pevně určí výšku okna dopředu (seznam, který roste a zase se zmenší, si drží jednu velikost panelu); když ho vynecháš, panel se přizpůsobí předaným řádkům.
...
- Čekací primitiva (ve skriptu s `yield from`): `.text(lines)` (stránka dialogu, A pokračuje) · `.ask(lines) -> nastaví .answer` (volba A/B) · `.wait(frames)` · `.fade_out(speed)` / `.fade_in(speed)` · `.goto(level, at=None)` (fade out, nastaví `.pending`, počká, až herní smyčka vymění úroveň, fade in).
- Příběhové flagy: `.ev(name)` / `.set(name)` (nastaví A přehraje efekty úrovně přes `.on_flag`) / `.ev_set(name)` (jen nastaví) / `.ev_clear(name)`, uložené v `.events` (množina — ulož ji vlastním save schématem). `.view` = aktuální View (udržuje `picogame_story`). `.retarget(scene)` přesměruje Director po výměně mapy; `.pending` = `(level, at)`, o které skript požádal.
...
- `Save(key, schema, *, offset=0)` — `schema` = uspořádaný dict `jméno -> (formátový znak struct, výchozí)`; ukázka → [/cs/helpers/data/](/cs/helpers/data/). · `.defaults()` · `.load() -> dict` · `.save(values)` · `.reset()`. Přežije restart/smazání souborového systému.

### `picogame_audioout` — jeden výstup pro libovolnou desku
- `make_output(sample_rate=22050, pin=None)` — vrátí audio výstup desky, vybraný automaticky: I2S DAC (Fruit Jam TLV320), když má deska `I2S_BCLK` a nepředáš `pin`, jinak analogovou cestu na `pin` (nebo výchozím pinu desky) — PWM tam, kde firmware má `audiopwmio`, a pravý DAC čipu tam, kde ho nemá (SAMD51: PyBadge, PyGamer). Používá ho `picogame_audio` i `picogame_synth`, takže hra nepotřebuje žádný kód specifický pro desku. Vyhodí `RuntimeError`, pokud výstup neexistuje.
...
- `encode_pal8(data, fw, fh, frames, palette, transparent) -> bytes` — soubor `.pal8` (16bajtová hlavička, paleta, indexy; konstanty `PAL8_MAGIC` / `PAL8_HDR` / `PAL8_HDR_LEN` jsou právě ta hlavička, rozepsaná ve [Formátu scény](scene-format.md)). Zabírá ~3 kB, dokud je importovaný; `Game` ho po startu zahodí.
...
- `Road(pg, width, height, horizon, colors, *, half_width=0.47, hw_min=6.0, depth=600.0, curves=((16384, 90.0), (4096, 30.0)), world_step=6, curve_step=2, hill_amp=0, edge_frac=0.12, dash_frac=0.07, dash_min_hw=7.0, band=20.0, dash_band=14.0)` · `.tick(dist, lateral_px=0)` (1×/snímek; kladné lateral = auto jelo doprava, silnice se posune doleva) · `.draw(view, vy)` (tělo StripDraw callbacku) · `.set_grade(g)` (kopce: −1..+1 posouvá horizont; chce `hill_amp`) · `.horizon_now` (efektivní horizont tohoto snímku = horizont + sklon kopce — y, ke kterému se kotví overlaye a sprity u krajnice) · `.curve_at(dist)` (znaménková křivost −1..+1 pro odstředivou sílu / AI — stejný dvousinový model, který běží v C, bez alokace) · `.row_of(z)` / `.half_of(row)` / `.edges_of(row)` (umístění a škálování spritů NA silnici — řádky jsou lineární, škáluj přes `half_of`, ne `F/(F+z)`) — řídí nativní pár `pg.road_edges` + `Canvas.road` z lidských jednotek: `periody` zatáček ve světových jednotkách (zaokrouhlené na nejbližší mocninu dvou, aby int32 fázový wrap zůstal spojitý — viz [smlouvu volání](/cs/helpers/pseudo-3d/)), `swing` v px bočního ohnutí, barvy jako dict šesti hodnot `pg.rgb565`. Všechny fixed-point tabulky staví jednou; `tick()` nealokuje. Viz [/cs/helpers/pseudo-3d/](/cs/helpers/pseudo-3d/).

### `picogame_iso` — izometrická projekce
- `IsoView(ox, oy, tw, th)` (`tw`/`th` = poloviční šířka/výška dlaždice; 2:1 diamant → `th = tw//2`) · `.to_screen(gx, gy, h=0)` · `.depth(gx, gy, h=0)` (painter's klíč zezadu dopředu) · `.screen_to_grid(sx, sy)` · `.cube_faces(gx, gy, height_px)` (horní/pravá/levá stěna vyvýšeného bloku) · `.emit_blocks(cells, tv, tc)` (alloc-free dávka: zapíše flat-shaded trojúhelníky kostek mnoha bloků přímo do int16/uint16 bufferů pro JEDNO volání `Canvas.fill_triangles`; vrací počet trojúhelníků) — **nejlevnější pseudo-3D vůbec**: jen celočíselné sčítání a shifty, žádné dělení, žádná závislost na C, proto běží dobře i na RP2040. Odemyká iso RPG / strategie / taktiky / buildery. **Bloky kresli jako Sprites** (jedna zapečená Bitmap na typ bloku, přidané v pořadí podle `depth()`), pokud není geometrie procedurální: změřeno na PicoPadu se 150 pohybujícími se bloky stojí sprity ve scéně 40 ms na snímek proti 89 ms u přestavby přes `emit_blocks` a `Triangles`. Statická deska je ještě levnější: vykresli jednou + dirty-rect jen pohyblivé (30 fps). `emit_blocks` je dávkový builder pro scény, jejichž geometrie se opravdu počítá každý snímek (~2× rychlejší než Python smyčka přes `cube_faces`). Viz [/cs/helpers/pseudo-3d/](/cs/helpers/pseudo-3d/).
