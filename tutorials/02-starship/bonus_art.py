...
import art_ship                               # <-- generated from a CC0 PNG
import art_bullet
...
ship_bm = art_ship.bitmap(pg)                 # <-- was shp.poly_frames(...) : real ship, 16 frames
DIRS = [(math.sin(f * 2 * math.pi / NF), -math.cos(f * 2 * math.pi / NF)) for f in range(NF)]
bullet_bm = art_bullet.bitmap(pg)             # <-- was shp.circle(...) : real laser tile
