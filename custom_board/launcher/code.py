# code.py — LAUNCHER for a stock picogame firmware (no board.DISPLAY slot).
# Builds the display from settings.toml, makes it board.DISPLAY, then runs your UNCHANGED game
# (game.py, or PICOGAME_GAME in settings.toml). Self-contained: no extra lib. Copy this + settings.toml
# + your game (game.py) to CIRCUITPY. If you can flash the custom-board firmware, prefer that path
# (firmware_boot/) — there the game is code.py and you need no launcher.
#
# settings.toml: same PICOGAME_DISPLAY/PINS/SIZE/FLIP/INVERT/BGR/BAUD keys as firmware_boot/, plus:
#   PICOGAME_GAME = "game"    (the module to run)
import sys
import os
import board
