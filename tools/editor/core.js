...
(function (root) {
  ...
  // ---------------------------------------------------------------- problems (validation)
  // The editor-side twin of tools/scene_build.py's validate(): every check a person can act on
  // from the panels, run against the LIVE model (no export, no clone) so a panel can call it on
  // every render. Checks that need the disk (a missing PNG, story.py, canonical formatting) stay
  // in the CLI. -> [{severity, level, where, msg, sel}], errors first; `sel` is what the UI should
  // select when the row is clicked (the object itself, or an asset id).
  const IDENT_RE = /^[A-Za-z_][A-Za-z0-9_]*$/;
  function isIdent(s) { return typeof s === "string" && IDENT_RE.test(s); }
  function q(s) { return "'" + s + "'"; }
  function nth(i) { return "#" + (i + 1); }

  function problems(project) {
    const knowScripts = !!(project.scripts && Object.keys(project.scripts).length);
    const errs = [], warns = [];
    const assets = (project && project.assets) || {};
    const levels = (project && project.levels) || [];
    const used = Object.create(null);              // asset id -> placed/painted somewhere
    function err(level, where, msg, sel) { errs.push({ severity: "error", level: level, where: where, msg: msg, sel: sel || null }); }
    function warn(level, where, msg, sel) { warns.push({ severity: "warning", level: level, where: where, msg: msg, sel: sel || null }); }

    // ---- the shared asset bank
    for (const id in assets) {
      const a = assets[id], where = "assets[" + q(id) + "]", sel = { kind: "asset", ref: id };
      if (!isIdent(id))
        err(null, where, "the asset id " + q(id) + " is not an identifier — rename it to letters, digits and _ (not starting with a digit)", sel);
      if (isImg(a)) {
        if (!a.src)
          err(null, where, "has no file name — give it the PNG that ships next to game.json", sel);
        else if (/[\\/]/.test(a.src))
          err(null, where, "the file name " + q(a.src) + " has a path in it — the art sits next to game.json, so use a bare name", sel);
      }
    }

    // ---- level names + the start level
    const byName = Object.create(null);
    levels.forEach(function (lv, li) {
      const sel = { kind: "level", ref: lv };
      if (!isIdent(lv.name))
        err(li, "level " + q(lv.name), "the level name " + q(lv.name) + " is not an identifier — Save has to rename it to " + q(slugName(lv.name)) + "; rename it here to pick the name yourself", sel);
      if (byName[lv.name])
        err(li, "level " + q(lv.name), "there is already a level called " + q(lv.name) + " — level names are the game's keys, so give this one its own", sel);
      else byName[lv.name] = lv;
    });
    if (project && project.start && !byName[project.start])
      err(null, "start level", "the game starts on level " + q(project.start) + ", which does not exist — pick one of the levels", null);

    levels.forEach(function (lv, li) {
      const lsel = { kind: "level", ref: lv }, lwhere = "level " + q(lv.name);
      const seen = Object.create(null);            // label -> where of the object that took it
      const labels = Object.create(null);          // what hide/show and the camera can name
      const groups = Object.create(null);          // tags already folded into a group
      const world = levelBounds(project, lv);
      function claim(label, where, sel) {
        if (!label) return;
        if (seen[label])
          err(li, where, q(label) + " is taken — " + seen[label] + " already uses it, and a name or tag " +
                        "has to be unique inside a level (the game looks objects up by it)", sel);
        else seen[label] = where;
      }

      // sprites / groups
      (lv.entities || []).forEach(function (en, i) {
        const plain = en.tag && !en.name && !en.data && !en.frame && !en.angle;   // folds into a group
        const where = plain ? "group " + q(en.tag)
          : en.name ? "sprite " + q(en.name)
          : en.tag ? "sprite " + nth(i) + " (tag " + q(en.tag) + ")"
          : "sprite " + nth(i) + " (" + q(en.asset) + ")";
        const sel = { kind: "entity", ref: en };
        if (plain) {
          if (!groups[en.tag]) { groups[en.tag] = true; labels[en.tag] = true; claim(en.tag, where, sel); }
        } else if (en.name) { labels[en.name] = true; claim(en.name, where, sel); }
        const a = assets[en.asset];
        if (!a) {
          err(li, where, "is drawn from " + q(en.asset) + ", which is not in the asset bank — pick an asset, or add that one", sel);
          return;
        }
        used[en.asset] = true;
        const n = tileCount(a);
        if ((en.frame || 0) >= n)
          err(li, where, "shows frame " + (en.frame || 0) + ", but " + q(en.asset) + " has only " + n + " frame" +
            (n === 1 ? "" : "s") + " (0-" + (n - 1) + ") — pick a frame it has", sel);
        if (en.anim && !((a.animations || {})[en.anim]))
          err(li, where, "plays the animation " + q(en.anim) + ", which " + q(en.asset) + " does not define — add it to the asset, or clear it", sel);
      });

      (lv.particles || []).forEach(function (fx, i) {
        const where = "particles " + (fx.name ? q(fx.name) : nth(i));
        if (fx.name) labels[fx.name] = true;
        claim(fx.name, where, { kind: "particle", ref: fx });
      });

      (lv.hud || []).forEach(function (h, i) {
        const where = "hud label " + (h.name ? q(h.name) : nth(i));
        if (h.name) labels[h.name] = true;
        claim(h.name, where, { kind: "hud", ref: h });
      });

      (lv.points || []).forEach(function (pt, i) {
        const where = "point " + (pt.name ? q(pt.name) : nth(i));
        const sel = { kind: "point", ref: pt };
        claim(pt.name, where, sel);
        if (pt.x < 0 || pt.y < 0 || pt.x >= world[0] || pt.y >= world[1])
          warn(li, where, "sits at " + pt.x + "," + pt.y + ", outside the " + world[0] + "x" + world[1] + " world — move it in, or grow the world", sel);
      });

      // tilemap layers
      (lv.tilemaps || []).forEach(function (tm, i) {
        const where = "tilemap layer " + nth(i) + " (" + q(tm.asset) + ")";
        const sel = { kind: "tilemap", ref: tm };
        const a = assets[tm.asset];
        if (!a) {
          err(li, where, "paints with " + q(tm.asset) + ", which is not in the asset bank — pick a tileset, or add that one", sel);
          return;
        }
        used[tm.asset] = true;
        const n = tileCount(a), grid = tm.grid || [];
        const bad = [];
        let more = 0;
        for (let y = 0; y < grid.length; y++) {
          const row = grid[y];
          for (let x = 0; x < row.length; x++) {
            const t = row[x] & 0xFF;               // bits 8-10 are the orientation, not the tile
            if (t >= n) { if (bad.length < 3) bad.push("tile " + t + " at " + x + "," + y); else more++; }
          }
        }
        if (bad.length)
          err(li, where, bad.join("; ") + (more ? " and " + more + " more cell" + (more === 1 ? "" : "s") : "") +
            " — " + q(tm.asset) + " only has tiles 0-" + (n - 1) + "; repaint " + (bad.length + more > 1 ? "them" : "it"), sel);
      });

      // zones (story data / scripts / goto targets)
      (lv.zones || []).forEach(function (z, i) {
        const where = "zone " + (z.tag ? q(z.tag) : nth(i));
        const sel = { kind: "zone", ref: z };
        claim(z.tag, where, sel);
        if (z.x + z.w <= 0 || z.y + z.h <= 0 || z.x >= world[0] || z.y >= world[1])
          warn(li, where, "lies outside the " + world[0] + "x" + world[1] + " world, so nothing can ever walk into it — move it in, or grow the world", sel);
        const d = z.data;
        if (!d) return;
        const declarative = ("say" in d) || ("ask" in d) || ("goto" in d);
        if (("script" in d) && declarative)
          err(li, where, "has both dialogue data (say/ask/goto) and a script — a zone is one or the other; keep the script, or delete it and keep the data", sel);
        if ("goto" in d) {
          const g = Array.isArray(d.goto) ? d.goto : [d.goto];
          const tgt = byName[g[0]];
          if (!tgt)
            err(li, where, "goto names level " + q(g[0]) + ", which does not exist — send it to a level in this project", sel);
          else if (g.length > 1 && g[1] && !(tgt.points || []).some(function (p) { return p.name === g[1]; }))
            err(li, where, "goto arrives at point " + q(g[1]) + ", which level " + q(g[0]) + " does not have — add that point there, or drop it and arrive at the level's own spawn", sel);
        }
        if ("script" in d) {
          const s = d.script;
          if (!isIdent(String(s)))
            err(li, where, "runs the script " + q(s) + ", which is not a Python identifier — it becomes def " + s + "(d), so use letters, digits and _", sel);
          else if (knowScripts) {
            // Only judge this when the editor has story.py at all: a game.json opened without it
            // is the normal case, and every zone would otherwise be reported as broken.
            const body = Object.prototype.hasOwnProperty.call(project.scripts, s) ? project.scripts[s] : "";
            if (!String(body == null ? "" : body).trim())
              warn(li, where, "runs the script " + q(s) + ", which the Story panel has no body for — write it there (or it has to live in story.py next to the game)", sel);
          }
        }
      });

      // effects rules (they retile the level's first tilemap and hide/show its sprites)
      const tmAsset = (lv.tilemaps || [])[0];
      const tiles = tmAsset ? assets[tmAsset.asset] : null;
      const nTiles = tiles ? tileCount(tiles) : null;
      (lv.effects || []).forEach(function (rule, i) {
        const where = "effects rule " + nth(i) + (rule["if"] ? " (if " + rule["if"] + ")" : "");
        ["swap", "solid", "unsolid"].forEach(function (key) {
          (rule[key] || []).forEach(function (t) {
            if (typeof t !== "number")
              err(li, where, key + " lists " + JSON.stringify(t) + " — effects take tile numbers, not legend characters", lsel);
            else if (nTiles != null && t >= nTiles)
              err(li, where, key + " uses tile " + t + ", but " + q(tmAsset.asset) + " only has tiles 0-" + (nTiles - 1), lsel);
          });
        });
        (rule.hide || []).concat(rule.show || []).forEach(function (nm) {
          if (!labels[nm])
            err(li, where, "hides/shows " + q(nm) + ", but nothing in this level is called that — name a sprite, group, label or fx layer of this level", lsel);
        });
      });

      // camera
      const cam = lv.camera;
      if (cam && (cam.mode || "follow") === "follow") {
        if (!cam.target)
          warn(li, lwhere, "the camera is set to follow, but no target is chosen — name the sprite it should track", lsel);
        else if (!labels[cam.target])
          warn(li, lwhere, "the camera follows " + q(cam.target) + ", but nothing in this level is called that", lsel);
      }

      if (!(lv.tilemaps || []).length && !(lv.entities || []).length)
        warn(li, lwhere, "has no tilemaps and no sprites — it will show as an empty background", lsel);
    });

    for (const id in assets)
      if (!used[id])
        warn(null, "assets[" + q(id) + "]", "nothing uses " + q(id) + " — no layer paints with it and no sprite is placed" +
          " from it; place it, or delete it", { kind: "asset", ref: id });

    return errs.concat(warns);
  }
  ...
  const api = { newProject: newProject, newLevel: newLevel, newTilemap: newTilemap,
    ...
    bakeScene: bakeScene, sceneModule: sceneModule, problems: problems, pyRepr: pyRepr, PyTuple: PyTuple, PyBytes: PyBytes, PyFloat: PyFloat };
