...
description: >-
  ...
  settings.toml (PICOGAME_BUTTONS / MATRIX / SHIFTPAD / TILTPAD / INVERT / MADCTL / AUDIO / USBPAD ...), asking which GPIO a
...
- **`boot.py` runs only at power-on.** After copying a new `boot.py` (a code-built display on a bare
  board), press RESET once — a save or soft reload runs `code.py` but not `boot.py`, so the game finds
  no display until then.
...
- **C. Needs a rebuild**: the board's firmware lacks picogame or a backend it needs (framebuffer/DVI,
  DMA fast display, the RGB444 capability). Settings can't add those. Do Step 4.

**Completion:** you've named the path (A/B/C) and what hardware is attached.

## Step 1 — detect the wiring (bare/custom board)

**Prerequisite — the board must already run CircuitPython with picogame.** A bare Pico out of the box
runs nothing, and the probe + every template `import board` / `picogame_*`. First: flash a firmware with
picogame for the chip — stock CircuitPython for Pico / Pico W from 10.4 on, or the picogame downloads
(`pico` / `pico_w` / `pico2` / `pico2_w` — the RP2350 `pico2` has far more RAM), then copy to `CIRCUITPY/lib/` the current `picogame-libs/mpy/` bundle (delete any old
...
buttons (one-pin-per-button `PICOGAME_BUTTONS`, a scanned `PICOGAME_MATRIX_*` keypad, a shift register
`PICOGAME_SHIFTPAD`, an I2C pad `PICOGAME_I2CPAD`, an accelerometer `PICOGAME_TILTPAD`), display
(`PICOGAME_INVERT` / `PICOGAME_MADCTL` / `PICOGAME_BRIGHTNESS` / `PICOGAME_RGB444`, or the code-built
display for a bare board), audio (`PICOGAME_AUDIO` pin, or I2S `PICOGAME_AUDIO_OUT` + `_VOLUME`), USB gamepad.
...
Settings can't change a compile-time flag. When the board's firmware lacks picogame or a backend it
needs (DVI/framebuffer vs SPI, DMA fast display, the RGB444 capability), rebuild per **`references/firmware.md`** (which
...
| File | Pull it for |
|---|---|
| `references/settings.md` | every `settings.toml` key: buttons, matrix keypad, shift register, I2C pad, tilt, display, audio, USB gamepad — value formats + examples |
