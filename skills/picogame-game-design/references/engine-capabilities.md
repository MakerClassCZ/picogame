...
**Batch fills — never loop small draw calls per strip.** A StripDraw callback runs once per render
strip (30×/frame at `strip_h=8`), so a Python loop of `fill_rect`s inside it multiplies by
strips-per-frame (measured: it made a raycaster 8× slower). Precompute the shapes ONCE per change
...
EVENT (a HUD, a held dim, a static gradient, a CRT overlay) gets `StripDraw(..., always_dirty=False)`
and calls `sd.invalidate()` exactly when its state changes — never `always_dirty=True` (the default!)
for content that is usually static: a full-screen always-dirty StripDraw forces a full recomposite+push
every frame and defeats the scene's dirty-rect tracking for the whole game. With `always_dirty=False`
the overlay still repaints correctly wherever OTHER layers dirty the region under it.
`picogame_ui.SceneBox`, `fx.Fade/Sky/Scanlines` and `picogame_ray.attach()` all follow this idiom.
...
| `picogame_input` | `Buttons(..., usb=None)`: `.poll()`, `.is_pressed/.just_pressed/.just_released(mask)`, `.has(mask)`, `.repeat()`, `.clear()` (flush input on a state/menu transition), `.deinit()` (release the pins so a NEW `Buttons()` can claim them without a reload); masks `UP DOWN LEFT RIGHT A B X Y L1 L2 R1 R2 START SELECT ALL`; **backend auto** = CP `keypad` (HW debounce, no missed taps) else digitalio, **or a scanned ROW×COL matrix** (`settings.toml PICOGAME_MATRIX_ROWS/COLS/MAP`); per-board map via `board_id` profile or `PICOGAME_BUTTONS`. **Auto-attaches a USB HID gamepad AND a USB HID keyboard** as extra OR'd sources on USB-host boards (Fruit Jam) — games get pad/keyboard input with ZERO code change (`picogame_usbpad` / `picogame_usbkbd`, wired or 2.4 GHz dongle; not Bluetooth); `usb=False` off / `usb=True` force; remap via `PICOGAME_USBPAD`. Local multiplayer = one `Buttons` per player via `Buttons(sources=[…])` + `find_pads()` | all button input — portable across boards, incl. a USB gamepad/keyboard |
...
| `picogame_shapes` | generate PAL8 bitmaps (one colour, or a {character: colour} mask palette for multi-colour art): `rect/circle/ring/from_mask/masks/atlas/color_frames/tileset_colors/poly_frames` | stop hand-rolling pixel buffers; `tileset_colors` for solid-tile sheets; `poly_frames` to bake rotation frames |
| `picogame_pool` | `Pool(scene, bitmap, capacity, anchor=None, fixed=False)`: `.spawn()->sprite|None` (slot back in its baseline look = as at the first spawn, so post-construction set-up like `flip_y` sticks), `.baseline()`, `.free(s)`, `.free_all()`, `.count()`, `.items` | many of the **same** bitmap (bullets, sparks, pipes) — pre-allocate, never alloc per frame |
...
| `picogame_anim` | `FrameAnim(sprite, frames, *, fps=8, loop=True).tick(dt)`; `AnimatedSprite(sprite, {name:(frames,fps,loop)}).play(name)` then `.tick(dt)` per frame (`play` returns self and is a no-op when that animation already plays - safe to chain and to call every frame) | time-based / named-state animation (walk/idle/jump) |
| `picogame_ui` | **RAM first, then render context**: `SceneBox`/`SceneMenu`/`HudBar` are buffer-less StripDraws = **0 retained bytes** for the LAYER (text drawn through `picogame_font` - SceneLabel, HudBar labels - still grows a permanent per-glyph cache, a few KB for a game's vocabulary, which none of these figures include), while every `SceneLabel.reserve(n)` holds a text bitmap (reserve(34) ~ 2.4 KB, so a 3-label title screen costs ~7 KB = 5% of the RP2040 heap). A screen you only see between runs should be a SceneBox, not a stack of labels. Then: text/box/menu in two render contexts (see the **UI widgets** box below): scene-layer `SceneLabel`/`SceneBox`/`SceneMenu`, immediate `Label`/`TextBox`/`Menu`, plus `HudBar` (reserved-strip) + `GridCursor` (grid logic); `LINE_H=12` | HUD/dialog/menu-driven games (RPG, strategy) |
...
| `picogame_audio` | WAV/PWM: `Audio(pin=None, voices=4, sample_rate=22050, channels=1, bits=16, signed=True)` (needs `voices>=2`), `.load/.sfx/.music/.stop/.deinit`; `tone(freq, ms)` **builds** a RawSample you play via `.sfx(tone(...))`. Output auto-picked (see `picogame_audioout`) | recorded SFX / music; **each sample is resident RAM** |
...
| `picogame_synth` | synthio SFX/MIDI: `Synth()`, `note(...)`, `sfx(n, priority=0, window=0)` (priority classes + protected windows so hit-spam can't erase a boss boom), `sfx_seq(events)`, `Drone` (held engine/siren note fed per frame), `square(duty)` pulse timbres, `set_levels/mute`, `.deinit()` (release the output so a later `Synth()`/`Audio()` in the same program can claim it), `load_midi(...)`; ~0 RAM. **Imports + runs on ANY build** (audio-less/sim degrade to silent no-ops) and `Synth()` **self-guards a failed init** (tight heap / claimed pin) → check module `AVAILABLE` / instance `.available`, **no `try/except` needed** | a big bank of chiptune SFX or sequenced music |
...
| `picogame_save` | `Save(key, schema, *, offset=0)`: `.load()->dict`, `.save(dict)`, `.reset()` — NVM-backed (survives reboot + FS wipe). **Unlike `picogame_synth`/`picogame_sfx` it does NOT self-guard: the constructor raises `RuntimeError` on a build without NVM**, so wrap it - **with the import INSIDE the try** (`try:\n    import picogame_save\n    sv = picogame_save.Save(...)\nexcept Exception:\n    sv = None`); a bare `except Exception` around only the call swallows a forgotten import too, and the game then silently never saves or the game dies at startup on such a board | high scores / progress / settings; per-game `key`; write on game-over, not per frame |
| `picogame_fx` | `Shake(scene).add(amt)/.tick(camx,camy)`; **`Flash(sprite).hit(colour, frames)/.tick()`** — the 1-3 frame hit-flash WITHOUT the off-by-one (it counts DRAWN frames, and restores whatever tint/dither/shadow the sprite wore before); `Fade(scene, width, height, x=0, y=0, color=0, cell=8)` dither fade/dim/flash `.out()/.into()/.dim()/.pulse()/.tick()` (**`.out()` ends FULLY opaque** — for a message that must stay readable use `.dim()`); `Tween(v).to(t).tick()`; `Camera(scene, w, h, lerp=0.18, world_w=0, world_h=0, top=0, bottom=0, left=0, right=0).follow(x,y).apply()` (**pass `world_w`/`world_h` by keyword** — positional args after `h` set `lerp` and break the follow; `w`/`h` = the screen, a HUD band goes in `top=`/`bottom=`/… — the same band as `setup()` — NOT into `h`); `Sky`, `Scanlines`, `InvertFlash` | JUICE: screenshake, scene transitions / menu dim, value easing, smooth follow camera (composes with Shake), gradient sky, CRT scanlines, full-screen invert flash |
...
| `picogame_arena` | `Arena(pixels)`: `.alloc(nbytes, align=1)->memoryview`, `.canvas(w,h,transparent=None)`, `.reset()`, `.free()`, `.mark()/.release(m)` (nested LIFO lifetimes) | dodge heap fragmentation — grab one big buffer up front, slice it (needs firmware `Canvas(buffer=)`) |
...
| `picogame_scene` | `Game(pg, "game.json", font=…)` streams a game.json and bakes every level at boot (RP2040: 40–60 ms, ~4.5 kB per level), `.load(name, at)->View`, `.levels/.start`; `Game(pg, "game_bank")` = the shipped `.mpy` bank; `load(pg, scene, …)` for a baked SCENE dict; `View`: `.is_solid/.tile_has/.point/.group/.in_zone/.tile_xy/.play/.tick/.swap_tiles/.effects/.world` | data-driven levels from the web editor (`docs/scene-format.md`); iterate from JSON, ship with `scene_build.py build --mpy` |
| `picogame_story` | `Story(d, game, story_module)`: `.enter(view, x, y)` starts the zone you just entered (say / ask / goto data or `{"script": name}` -> `def name(d)` in story.py), `.effects(view)` replays the level's rules; `Director.set(flag)` replays them, `yield from d.goto(level, point)` asks the loop to swap levels (two-phase: unload, `gc.collect()`, `game.load`) | zones with dialogue, one-shot switches, guarded exits and world changes with no code; story.py for the rest |
...
| `picogame_tiles` | `TileFlags(flags, tile_px=8)`: `.get(tile,bit=None)`, `.set(tile,bit,value=True)`, `.at(tm,tx,ty,bit)`, `.at_px(tm,px,py,bit)`. `B_SOLID/B_HAZARD/B_LADDER/…`=bit INDICES (for get/set/at/at_px); `SOLID/HAZARD/…`=masks (only for the `{tile:flags}` table). **8 bits total** (one byte per tile), all eight redefinable — `B_CUSTOM` is just the unnamed one; a ninth property goes into scene props / a second table, not here | gameplay properties per tile (solid/hazard/ladder) without a parallel map |
...
reserved border (0 RAM, `draw()` on change; update a text field with the `label(...)` handle's `.set(text)` / `.color(fg)`); `GridCursor` = grid-cursor LOGIC only (`tick→(tx,ty)`),
...
clock = picogame_clock.Clock(30)                      # 30 = the RP2040 baseline (§ RAM/perf)
dt = 1 / 30                                          # seed BEFORE the loop (first frame uses it)
while True:
    btn.poll()
...
for b in bullets.items:                   #  for several, pre-allocate a dict per slot at startup
    ...
    if b.fy < -8: bullets.free(b)         #  fresh tuple/dict per change (a rewritten 3-tuple
                                          #  = per-frame GC churn (KBs/frame at tens of entities)
...
scene.add(pg.StripDraw(road, 0, 0, 320, 240))          # screen-space always; fixed= is a no-op on it
```
...
- **Ship `.mpy`, not big `.py`.** CircuitPython compiles `code.py` at boot; a large source file's
  ...
  and precompile with `mpy-cross` **matching the firmware's mpy/CP version** (see `docs/firmware.md`; a mismatched `.mpy` won't import).
...
- A shmup load (player + ~8 enemies + ~14 bullets + ~180 particles) runs ~35–40 fps uncapped on RP2040; a static background + localized motion → 100+ FPS.
...
- **Math:** `x*x` not `x**2`; `math.sqrt(d)` not `d**0.5` (`**` goes through generic pow — measurably slower, but neither shows up next to rendering).
...
- **Prompt explicitly:** the engineering part is fixed — "pixel art, <view: top-down from directly
  above / side view>, <silhouette>, centered, plain background" — the style words are YOURS
  ("dark body, single glowing accent" was SALVO's style, not the recipe). Vague top-down prompts get
  misread (a "top-down eel" came back as a stick).
...
| **Rotation: runtime vs pre-baked** | `games/picoracer` and `demos/picogame_asteroids.py` (both runtime `sprite.angle`) vs `games/bangbang` (pre-baked `shapes.poly_frames` frames). TIE-BREAK when a sprite is both few-of-a-kind and always turning: bake it if the angles are few and fixed (8/16 headings) or the art must stay crisp; use `angle` if the rotation is continuous or the atlas would not fit - baked frames cost RAM once, `angle` costs CPU every frame. |
...
- **RED FLAGS — measure BEFORE building further** (each alone is fine; stacked they sink RP2040):
  ...
  refresh has a hard ~24 ms floor (~19 ms with `rgb444="auto"` on ST7789), and per-frame Python
...
- **You don't need fixed-point in game logic.** Write plain floats (`sprite.fx/fy` are sub-pixel) —
  ...
  (Why the rule exists, in numbers: PicoLibSDK's measured timing tables —
  breatharian.eu/hw/picolibsdk/dwn/Timing.htm — put soft-float ops on the M0+ at hundreds of ns
  to µs each vs single-digit ns on the RP2350 FPU; that gap is the whole story. Don't churn
  floats *per-item* in M0+ Python loops either — each op goes through the same soft-float path.)
...
- **Anchor + rotation/scale interplay.** `scale`/`angle` pivot about the `anchor` (fractions of the
  ...
  correctly on both sides (the old Python collision helper that did not is deleted). `scale=1.0, angle=0` is the fast blit path — leave them there when idle.
