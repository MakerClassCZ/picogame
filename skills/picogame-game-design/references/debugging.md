...
| Draw/fill phase is SLOWER on a Fruit Jam than on a PicoPad (the faster board!) | The Python heap lives in **PSRAM** on the Jam — retained-Canvas fills pay external-memory writes (~7× slower than SRAM; measured 8.7 vs 64 MB/s). Repainted-every-frame surfaces belong in a **StripDraw**: emit a screen-space triangle batch once, replay per strip with `view.fill_triangles(tv, tc, n, 0, -vy)` (full res, 0 retained RAM). Keep retained canvases small or static on fb boards. |
