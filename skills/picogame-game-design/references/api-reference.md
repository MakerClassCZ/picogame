...
- `remove(item)` — unlink a previously added item (no ghost — next refresh repaints over it, like `invalidate(x=0, y=0, w=0, h=0)` (the rect repaints only that sub-region on device - a menu row, a card slot; the SIM accepts the args and ignores them, having no dirty-rect)); the item survives and can be `add()`ed again. `ValueError` if not in the scene.
...
Immediate-mode layer with **no pixel buffer**: each refresh it calls `callback(view, vx, vy, vw, vh)` once per render strip its rect touches. It has **no `visible` flag** (unlike Sprite/Tilemap): hide one by setting `height = 0` (that also stops the per-strip callback cost) and restore the height to show it. **`vw`/`vh` are the RENDER REGION's size, not the layer's** - the rect only decides which strips fire the callback, NOT how wide you may paint, so `view.clear()` or a full-view fill on a narrow layer wipes the whole screen width (paint your own rectangle with `fill_rect` instead). `view` is a Canvas pointing at the live strip (use Canvas primitives, incl. `view.text`); view-local `(0,0)` = screen `(vx, vy)`. In a scrolling scene add it `fixed`.
...
- `text_width(font, s) -> px` · `centred(s, chars) -> str` — the fixed-cell metric (6 px/char with the bundled font) and the pad-to-centre helper. Use `centred()`, **not `str.center()`**: str.center needs an EXTRA_FEATURES MicroPython build and is compiled OUT of CircuitPython - it works in the sim and raises on the device.
