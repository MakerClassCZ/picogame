...
Immediate-mode layer with **no pixel buffer**: each refresh it calls `callback(view, vx, vy, vw, vh)` once per render strip its rect touches. **`vw`/`vh` are the RENDER REGION's size, not the layer's** - the rect only decides which strips fire the callback, NOT how wide you may paint, so `view.clear()` or a full-view fill on a narrow layer wipes the whole screen width (paint your own rectangle with `fill_rect` instead). `view` is a Canvas pointing at the live strip (use Canvas primitives, incl. `view.text`); view-local `(0,0)` = screen `(vx, vy)`. In a scrolling scene add it `fixed`.
