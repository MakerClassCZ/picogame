...
- `screen() -> (width, height)` — the screen size; `display()` — the display object. Both read `supervisor.runtime.display` (the primary display - set by the firmware, or published by boot.py) and fall back to `board.DISPLAY` for the simulator/playground, so the same game runs on a PicoPad, a Fruit Jam and a bare Pico. Lay games out from `screen()`, never from a hardcoded 320×240.
