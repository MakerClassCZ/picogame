...
- Transform props (nearest-neighbour, about the anchor):
  ...
  - `angle` — rotation in degrees; `0` = none (fast path). **POSITIVE turns CLOCKWISE on screen** (screen y grows down), about the `anchor` — default `(0, 0)` = the top-left, so set `anchor = (0.5, 0.5)` to spin about the middle. Combines with `scale`.
...
- Blit-effect props (one at a time - a TRUTHY write to any of them clears the other three; a FALSY write (`spr.flash = 0`) clears ONLY that effect, so `spr.dither = 0` will NOT switch off an active flash - clear the effect you actually set. Watch COMPUTED values: `spr.dither = int(7 * (1 - frac))` is a FALSY write while the expression rounds to 0, so an earlier `flash` keeps painting - and then the bug heals itself as the value grows, which is the worst kind to catch on a screenshot; cheap, no extra bitmaps):
...
- `remove(item)` — unlink a previously added item; the removal forces a FULL repaint next refresh (no ghost). `Scene.invalidate()` takes NO arguments — the sub-rect form belongs to `StripDraw.invalidate`.
...
- `display` — the display/framebuffer the scene was built on (read-only in practice). Widgets take it from here (`picogame_ui` does), so a helper needs the scene, not a second display argument.
- `refresh() -> list | None` — diff & repaint changed regions; returns the dirty rect `[x1,y1,x2,y2]` (reused) or None. **The SIMULATOR has no dirty-rect**: it clears and redraws everything, then returns the full play rect — or `None` when the finished frame is pixel-identical to the last one (firmware parity for the `if scene.refresh():` idiom). So the rect's SIZE is not a measurement there (an instrumented "94% of the screen dirty" is a constant, not your game), but its truthiness is. Run `sim/run.py --strict-dirty` to at least catch the StripDraw half of it - a layer you forgot to `invalidate()` then freezes in the sim as it does on device.
...
`fade=True` DIMS each particle's colour as it ages (it does not just remove it), so a spark passes through shades that are in no palette of yours - budget for that when you're matching a signature palette.
`gravity` is px/tick² added to vy each tick, clamped by the C to **-128..127.99**; NEGATIVE gravity is legal and makes particles RISE (embers, bubbles, smoke). Particle positions are **world** coordinates - `scene.set_view` moves them like any other layer, so emit at world x/y, not screen x/y.
A pooled particle layer (small moving dots) drawn as one Scene layer. **Limits:** `capacity` 1..4096, `size` 1..8 px.
- `emit(x, y, count, speed=1, life=30, color=0xFFFF)` — burst `count` dots, random velocity ≤ `speed` px/tick, living `life` ticks. **Limits:** `count` ≥ 0, `speed` **0..127**, `life` 1..65535, `color` 0..0xFFFF (`x`/`y` are unrestricted — off-screen is fine). Anything computed from a delta or a velocity must be clamped BEFORE the call - and in a per-frame emitter write the clamp as `if v < 0: v = 0` / `elif v > 127: v = 127`, not `max(0, min(127, v))` (the builtin form is ~5x slower in a hot loop): a negative `speed` from e.g. `dx = target - source` raises `ValueError` at the moment the effect fires, which in a game means a crash on impact rather than at startup.
- `tick()` — advance one step (move, gravity, ageing). Call each frame.
- `clear()` — remove all.

### `Canvas(width, height, *, transparent=None, buffer=None)`
A RAM RGB565 drawing surface composited as a Scene layer (`width*height*2` bytes). `transparent` makes it a shaped overlay; `buffer` backs it with external memory (e.g. an arena slice); it must hold `width*height*2` bytes and start at an EVEN (2-byte aligned) address — a memoryview sliced at an odd byte offset raises. For *animated full-frame* surfaces prefer `StripDraw` (no buffer). **Limits:** `width`/`height` 1..1024; a supplied `buffer` must hold `width*height*2` bytes and have an even length. Drawing calls CLIP — off-surface coordinates are dropped, not an error.
...
- `text(x, y, s, fg, font, bg=None)` — composite a string in C, rasterizing each glyph of `font` (a `fontio.BuiltinFont`) on the fly: no Python glyph cache and no per-call Bitmap/Sprite. `bg=None` → transparent glyph background. ASCII/built-in font only. Works on a Canvas or a `StripDraw` view; the latter does not retain a separate text surface. **The built-in `terminalio.FONT` is a fixed 6×12 cell**, so a string is `len(s) * 6` px wide and centring is `(W - len(s) * 6) // 2` (both fonts are fixed-width: `picogame_bitfont` is 8×8). At 6 px/char a 320 px screen holds 53 characters and a 240 px one holds 40 — budget text against the smaller.
- `mode7(texture, horizon, y_off, z, rx0, ry0, rsx, rsy, cam_x, cam_y)` — fill the rows below `horizon` with a **Mode-7 perspective floor** of `texture` (power-of-2 dims; one world unit = one tile). `texture` is a Bitmap, `horizon`/`y_off` are pixel rows, the remaining 7 are 16.16 fixed-point — you normally let `picogame_mode7.Camera` compute them from a camera pose. Draws into a Canvas or a 0-RAM `StripDraw` view (pass `y_off` = the strip top).
...
Immediate-mode layer with **no pixel buffer**: each refresh it calls `callback(view, vx, vy, vw, vh)` once per render strip its rect touches. `scene.add(sd, fixed=True)` is a NO-OP for a StripDraw: the kind is screen-space in both the draw path and the dirty pass, so it never scrolls and never smears either way. It has **no `visible` flag** (unlike Sprite): hide one by setting `height = 0` (that also stops the per-strip callback cost) and restore the height to show it. **`vw`/`vh` are the RENDER REGION's size, not the layer's** - the rect only decides which strips fire the callback, NOT how wide you may paint, so `view.clear()` or a full-view fill on a narrow layer wipes the whole screen width (paint your own rectangle with `fill_rect` instead). `view` is a Canvas pointing at the live strip (use Canvas primitives, incl. `view.text`); view-local `(0,0)` = screen `(vx, vy)`. In a scrolling scene It is screen-space ALWAYS, so `fixed=True` on it is a no-op.
...
- `invalidate(x, y, w, h)` / `invalidate()` — mark dirty for the next refresh (the way to update an `always_dirty=False` panel). No args = the whole layer; ALL FOUR args = one view-local sub-rect (a menu row, a card slot; accumulates, clamped to the layer). Passing 1-3 args raises — all-four-or-none, positional. The SIM accepts the rect but full-repaints regardless (no dirty-rect there).
...
- The ONLY property is `count` — how many triangles the next refresh draws (assignment marks the layer dirty). There is no x/y/width/height/always_dirty on this type.
...
- `vblank(framebuffer)` — (DVI boards, RP2350) pass the `picodvi.Framebuffer` the display scans out of; blocks until its next vertical blanking (≤ ~16.7 ms). Starting a full-frame compose right after vblank keeps the publish front consistently behind the beam, so each sweep shows one **whole** frame — removes single-buffer tearing while the compose fits within two sweeps. Costs the wait: budget it against your FPS cap.
...
- `Timer(frames)` — input-leniency window (coyote time / jump buffering): `.feed(condition)` (recharge while true, else decay) · `.charge()` (fill) · `.is_active` · `.consume()` (true once, then clears) · `.t` (frames left). No `reset()`: `consume()` or `.t = 0` clears it — do that to the coyote timer right after a jump so it can't fire twice.

### `picogame_usbpad` — USB HID gamepad source (USB-host boards, e.g. Fruit Jam)
- `UsbPad(dev=None, vid=None, pid=None, ep=0x81, report_len=8, buttons=None, axes=..., timeout_ms=None)` — pass everything by keyword; a button **source** for `Buttons(usb=…)` (auto-attached by default on a USB-host build). Reads a USB HID gamepad and ORs it into the button mask, so a plugged-in pad works with **zero game code changes**. Needs a USB-host CircuitPython build (`usb.core`); a no-op on boards without it.
...
- `text_width(font, s) -> px` · `centred(s, chars) -> str` — the fixed-cell metric (6 px/char with the bundled font) and the pad-to-centre helper. `str.center` DOES exist on every picogame board (FULL_BUILD-gated; RP2/espressif are FULL_BUILD - HW-confirmed on the PicoPad). `centred()` earns its keep differently: it TRUNCATES to the reserve width, so an over-long string can't grow-realloc the label; and it survives a small-flash build lacking str.center.
- `HudBar.label(...) -> HudLabel` with `.set(text)` · `.color(fg)` — recolour a HUD field (a gauge that goes red when low) without touching the bar.
...
- `from_mask(mask, color)` — Bitmap from a **list of strings**, one per row (`'#'`, `'X'` or `'1'` = set); sized to the mask. `color` may also be a **{character: colour} dict** - then every character is its own palette entry (unlisted characters, e.g. `.` or space, stay transparent), so ONE mask carries body + outline + highlight: `from_mask(["  ##  ", " #oo# ", "#o..o#"], {"#": OUTLINE, "o": BODY, ".": GLINT})`. That is how you get the shape AND colour identity the design bar asks for without hand-building a PAL8 atlas. A single string duck-types through and makes each CHARACTER a row — a 1-pixel-wide sprite, no exception, so only a screenshot catches it.
- `masks(mask_list, color)` — **multi-frame `from_mask`**: a list of string masks → one horizontal atlas Bitmap (frame *i* = `mask_list[i]`), sized to the largest; takes the same {character: colour} palette. The step between `from_mask` (single frame) and `atlas` (raw 0/1 buffers, which nothing else produces).
...
- `poly_frames(size, points, nframes, color, fill=True)` — bake `nframes` rotations of a polygon. **Units:** `size` is the bitmap SIDE (the result is SQUARE, size x size per frame) and `points` are pixel offsets from its CENTRE, so a tall thin shape costs a bitmap as wide as it is tall - hand-write mask rows for those instead.

### `picogame_pool` — reusable sprite pool
- `Pool(scene, bitmap, capacity, anchor=None, fixed=False)` · `.spawn() -> sprite | None` (the slot comes back in its BASELINE look: blit effect, scale/angle, frame and flips restored to how the sprites looked at the first `spawn()` - so set-up right after construction is kept; `.data` and position are yours; `.baseline()` re-snapshots after a later reconfiguration) · `.free(s)` · `.free_all()` · `.count() -> int`. (`.items` = all sprites.)
...
- `Shake(scene, max_offset=6, decay=0.03, seed=0x9E37)` · `.add(amount)` (0.6 small kick, 0.8 hit, 1.0 big; under ~0.5 is sub-pixel at `max_offset=6` — invisible, only costs the repaints) · `.tick(cam_x=0, cam_y=0) -> bool` (True WHILE shaking) — trauma screen shake composed on top of the camera. **With a scene, call `tick()` EVERY frame**: it is what applies the camera (`set_view(cam_x + ox, cam_y + oy)`), idle or not, so gating it with `if shaking:` freezes a moving camera. The `if shaking: shaking = sh.tick()` short-circuit is only for a FIXED camera or `Shake(None)`, where an idle tick has nothing to apply. `scene=None` = offset-only mode for strip-rendered games (road/raycaster/mode-7, which `set_view` never moves): tick() just updates `.ox`/`.oy` and you spend them in your renderer's params — `road.tick(dist, lateral + sh.ox)`, a jittered horizon.
- `Flash(sprite)` · `.hit(color, frames=2)` · `.tick() -> bool` — the 1-3 frame hit-flash, counted in DRAWN frames. A hand-rolled counter set in the collision pass and decremented in the entity pass renders 2 as ONE frame; this also restores the tint/dither/shadow the flash replaced (they share one slot). Call `tick()` once per frame after the logic.
- `Fade(scene, width, height, x=0, y=0, color=0, cell=8)` · `.to(target, speed=2.0)` · `.out()/.into()/.set(level)/.dim(level=8)/.clear()/.pulse(level=12, speed=2.0)` · `.is_done` · `.tick() -> done` — dither fade / dim / flash, full-screen or a region. Uses `StripDraw` without a retained pixel surface. There is no alpha on this device: a dim reads as a **stipple/checkerboard**, not a smooth veil - larger `cell` = coarser blocks, so a readable menu dim is roughly `cell=4` + `dim(5)`, and a screenshot is the only way to judge it.
- `Tween(value=0.0, speed=0.2)` · `.to(target, speed=None)` · `.set(value)` · `.tick() -> value` · `.is_done` — ease a scalar (UI/pop-ups).
- `Camera(scene, w, h, lerp=0.18, world_w=0, world_h=0, top=0, bottom=0, left=0, right=0)` · `.follow(tx, ty, snap=False)` · `.apply(shake=None)` · `.offset() -> (ox,oy)` · `.ox`/`.oy` — smoothed follow + world clamp; `cam.apply(shaker)` composes with a `Shake` allocation-free (one `set_view`); `.offset()` allocates a tuple per call — not for the frame loop. `w`/`h` = the SCREEN; a reserved HUD band goes in `top`/`bottom`/`left`/`right` (the same numbers as `setup(top=…)`), which centres AND clamps inside the visible rect — the old `h=H+BAR` padding trick fixed only the centre and lost `BAR` px at both world edges.
- `Sky(scene, x, y, w, h, top, bottom)` — vertical gradient retaining ~`10*h` bytes of vspan run tables (five uint16 arrays of h entries). · `Scanlines(scene, x, y, w, h, step=2, dark=0)` — CRT overlay retaining one `w`-byte PAL8 row and its palette.
...
- `Rand(seed=None)` (deterministic combined 30-bit Lehmer PRNG; `None` = time-seeded) · `.below(n)` · `.randint(a, b)` · `.random()` · `.chance(p)` · `.choice(seq)` · `.shuffle(lst)` · `.weighted(weights) -> index` · `.seed(s)`.
