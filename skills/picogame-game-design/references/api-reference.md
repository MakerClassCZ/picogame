...
- `Pool(scene, bitmap, capacity, anchor=None, fixed=False)` · `.spawn() -> sprite | None` (the slot comes back in its BASELINE look: blit effect, scale/angle, frame and flips restored to how the pool was built - `.data` and position are yours; re-snapshot with `.baseline()` if you configure the sprites after construction) · `.free(s)` · `.free_all()` · `.count() -> int`. (`.items` = all sprites.)
